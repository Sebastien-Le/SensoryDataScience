...
```
Use *fill* to differenciate products: 
```{r}
ggplot(experts)+
  aes(x=Spicy, y=..ndensity..,fill=Product)+
  geom_histogram()+
  labs(title="Histogram of Spicy" ,x="Spicy values", y = "Density")
```
We use *ndensity* so scale at 1 maximum. But problem : densities are cumulated in the y-axis. We can change the position :
```{r}
ggplot(experts)+
  aes(x=Spicy, fill=Product)+
  geom_histogram(aes(y=after_stat(ndensity)), position=position_dodge())+
  labs(title="Histogram of Spicy" ,x="Spicy values", y = "Density")
```
Same example with only four products : 
```{r}
library(dplyr)
experts %>% filter(Product=="Chanel N5" | Product=="Pure Poison" | Product=="Shalimar" | Product=="Angel") %>%
  ggplot()+
  aes(x=Spicy, fill=Product)+
  geom_histogram(aes(y=after_stat(ndensity)), position=position_dodge())+
  labs(title="Histogram of Spicy" ,x="Spicy values", y = "Density")
...
```
It is possible for part of a curve to be >1 and for the area under the curve to be <1 or =1. Here, for the product "Pleasure", the part of the curve that is above y=1 --- i.e., the part between approx. x=0 and x=0.25 only has a width of around 0.25. So the area of that part is approximately 0.25 * 1.3 (1.3 being a guess at the average value in that x span), or around 0.325.You have to use ..scaled.. (density estimate, scaled to maximum of 1) within geom_density, by default it uses ..density...
...
```{r}
library(FactoMineR)
res<-PCA(scale(means.variables), scale.unit = F)
res$ind$coord
res$var$coord
```
