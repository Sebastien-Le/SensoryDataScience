...
#selection variable Political.Party and Position.A.Food
GMO.Al.A <- GMO[,c( "Political.Party","Position.A.Food")]

#or tidyverse version : 
library(dplyr)
GMO.Al.A <- GMO %>% select(c( Political.Party,Position.A.Food,))
```
...
#Re-order levels
GMO.Al.A$Position.A.Food <- as.factor(GMO.Al.A$Position.A.Food)
GMO.Al.A$Political.Party <- as.factor(GMO.Al.A$Political.Party)
GMO.Al.A$Position.A.Food <- ordered(GMO.Al.A$Position.A.Food,c("Totally opposed", "Somewhat Against", "Favourable", "Very Favourable"))
GMO.Al.A$Political.Party  <- ordered(GMO.Al.A$Political.Party,c("Extreme left", "Left", "Greens", "Liberal", "Right"))

ggplot(GMO.Al.A, aes(x=Position.A.Food, ..count..)) + geom_bar(aes(fill = Political.Party),  position = "dodge") + ggtitle("Barplot of cross-categories") +
...
GMO.Al.A.Sample <- data.frame("Position.A.Food"=GMO.Al.A$Position.A.Food[ind.sample],'Political.Party'=GMO.Al.A$Political.Party)

#Re-order levels
GMO.Al.A.Sample$Position.A.Food <- as.factor(GMO.Al.A.Sample$Position.A.Food)
GMO.Al.A.Sample$Political.Party <- as.factor(GMO.Al.A.Sample$Political.Party)
GMO.Al.A.Sample$Position.A.Food <- ordered(GMO.Al.A.Sample$Position.A.Food,c("Totally opposed", "Somewhat Against", "Favourable", "Very Favourable"))
GMO.Al.A.Sample$Political.Party  <- ordered(GMO.Al.A.Sample$Political.Party,c("Extreme left", "Left", "Greens", "Liberal", "Right"))

#visualization
ggplot(GMO.Al.A.Sample, aes(Position.A.Food, ..count..)) + geom_bar(aes(fill = Political.Party), color="black", position = "dodge") + ggtitle("Barplot of cross-categories sampled") +
  scale_fill_manual(values=c("Extreme left"="brown", "Left"="palevioletred2", "Greens"="chartreuse4", "Liberal"="gold1", "Right"="dodgerblue3"))
```
## probability side
...
#barplot
barplot(GMO_theo, beside = TRUE, main="Under the hypothesis of independence", col=c("brown","palevioletred2", "chartreuse4","gold1", "dodgerblue3"))

legend("topright", inset=.02, title="Political Party", c("Extreme left", "Left","Greens",'Liberal', 'Right'), horiz=F, cex=0.8, fill=c("brown","palevioletred2", "chartreuse4","gold1", "dodgerblue3"))
...
df <- df %>% pivot_longer(!Political.Party, names_to="Position.A.Food", values_to = "frequence")

#Re-order levels
df$Position.A.Food <- as.factor(df$Position.A.Food)
df$Political.Party <- as.factor(df$Political.Party)
df$Position.A.Food <- ordered(df$Position.A.Food,levels=c("Totally opposed", "Somewhat Against", "Favourable", "Very Favourable"))
df$Political.Party <- ordered(df$Political.Party, levels=c("Extreme left", "Left", "Greens", "Liberal", "Right"))

ggplot(df, aes(y=frequence,x=Position.A.Food, fill=Political.Party))+
...
Naturally, if you had to compare two political party in terms of difference from the independence model, you would calculate a distance based on the differences regarding each type of Position.A.Food categories. As the different types of Position.A.Food have a different weight relative to each other, you would naturally take that information into account, and calculate a distance weighted by the relative importance of each type of music. In other words, we're going to consider the following distance between two occupational statuses i and i':
