...
### Facteurs expérimentaux en 3 colonnes

fact_ex <- data.frame(matrix(ncol = 3, nrow = dim(orange)[1]))
colnames(fact_ex) <- c("Brand", "Pulpe", "Temperature")
...
for (i in 1:dim(orange)[1]){
  ...
  if(pdt_split[2]=="J"){
    ...
    fact_ex[i,1] <- "Tropicana"
  }
  if(pdt_split[3]=="P"){
    fact_ex[i,2] <- "With pulpe"
  }
  else{
    fact_ex[i,2] <- "Without pulpe"
  }
  if(pdt_split[4]=="A"){
    fact_ex[i,3] <- "Ambient"
  }
  else{
    fact_ex[i,3] <- "Refrigerated"
  }
}

orange_fact <- cbind(orange,fact_ex)
summary(orange_fact)
for (j in c(3,9:12)) orange_fact[,j] <- as.factor(orange_fact[,j])

res.mca <- MCA(orange_fact[,-c(1:2)], quali.sup = c(1,8:10))

## Récupération des modalité correspondant aux chaines de caractère que l'on souhaite représenter dans l'ACM
...
nb_session <- length(levels(goji$Session))
goji_attr <- as.data.frame(goji[,c(session,pdt,attr)])
tab <- as.data.frame(table(goji_attr[,1],goji_attr[,2],goji_attr[,3]))
colnames(tab) <- c("session","pdt","mod","count")    
list_graph <- list()
a=1

for (j in unique(goji[,pdt])){
  gg <- ggplot(tab[which(tab$pdt==j),-2],aes(x=mod,y=count,fill=session))+
    geom_bar(stat="identity",position=position_dodge())+
    theme_minimal()+
    ggtitle(str_glue("Distribution of attribute {colnames(goji)[attr]} for product {j}"))+
    xlab("Modalities")+
    ylab("Number of occurrences")+
    scale_fill_manual(values=c("orange","blue"))+
    ylim(0,max(tab$count))
...
if(length(unique(goji[,pdt]))%%2==0){
  plot_grid(plotlist = list_graph,ncol = round(length(unique(goji[,pdt]))/2,0), nrow=2)
}else{
  plot_grid(plotlist = list_graph,ncol = round(length(unique(goji[,pdt]))/2,0)+1, nrow=2)
}
